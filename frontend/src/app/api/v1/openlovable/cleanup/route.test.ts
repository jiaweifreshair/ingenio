...
describe('POST /api/v1/openlovable/cleanup', () => {
  ...
  it('应代理到后端并透传响应', async () => {
    const { POST } = await import('./route');
    const backendPayload = { success: true, data: { cleaned: true } };
    ...
    const res = await POST(makeRequest({ sandboxId: 'sb_456' }));

    expect(global.fetch).toHaveBeenCalledTimes(1);
    expect((global.fetch as any).mock.calls[0][0]).toBe(
      'http://backend.test/api/v1/openlovable/cleanup'
    );
