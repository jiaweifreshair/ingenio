...
export default function GitHubCallbackContent() {
      ...
      try {
        ...
        if (error) {
          ...
          setErrorMessage(
            error === 'access_denied'
              ? '您已取消GitHub授权'
              : `GitHub授权失败: ${errorDescription || error}`
          );
          toast({
            title: 'GitHub登录失败',
            description: errorMessage || 'GitHub授权失败',
            variant: 'destructive',
          });
        ...
        setStatus('error');
        setErrorMessage(
          error instanceof Error ? error.message : 'GitHub登录失败，请重试'
        );
        toast({
          title: 'GitHub登录失败',
  ...
  }, [searchParams, loginWithGitHub, router, toast, errorMessage]);
        ...
        <div className="bg-card rounded-lg shadow-lg p-8 text-center space-y-6">
          ...
          {status === 'loading' && (
            <>
              ...
              <h2 className="text-2xl font-semibold">正在完成GitHub登录...</h2>
