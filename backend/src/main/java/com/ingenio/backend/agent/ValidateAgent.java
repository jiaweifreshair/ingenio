...
/**
 * Validate Agent - 语义验证Agent
 * 负责验证AppSpec的完整性、一致性和合理性
 * 结合规则验证和AI语义验证，生成质量评分和优化建议
 */
@Slf4j
@Component
...
public class ValidateAgent {
     ...
     */
    public ValidateResult validate(Map<String, Object> appSpec) {
        if (appSpec == null || appSpec.isEmpty()) {
            log.error("ValidateAgent执行失败: AppSpec不能为空");
            throw new BusinessException(ErrorCode.PARAM_ERROR, "AppSpec不能为空");
        }

        try {
            log.info("ValidateAgent开始执行");

            // 第一步：规则验证（硬规则）
            ValidateResult ruleValidation = performRuleValidation(appSpec);

            // 如果规则验证发现严重错误，直接返回
            if (!ruleValidation.getIsValid()) {
                log.warn("ValidateAgent规则验证失败: errors={}", ruleValidation.getErrors().size());
                return ruleValidation;
            }

            // 第二步：AI语义验证（软规则）
            ValidateResult aiValidation = performAIValidation(appSpec);

            // 合并验证结果
            ValidateResult finalResult = mergeValidationResults(ruleValidation, aiValidation);

            log.info("ValidateAgent执行成功: isValid={}, qualityScore={}",
                    finalResult.getIsValid(), finalResult.getQualityScore());

            return finalResult;

        } catch (Exception e) {
            log.error("ValidateAgent执行失败: error={}", e.getMessage(), e);
    ...
    private ValidateResult performAIValidation(Map<String, Object> appSpec) {
        try {
            ...
            log.debug("ValidateAgent AI响应: {}", response);

            // 解析JSON响应
            return parseResponse(response);

        } catch (Exception e) {
            log.warn("AI语义验证失败，使用默认结果: error={}", e.getMessage());
            // AI验证失败时返回默认结果
            return ValidateResult.builder()
                    .isValid(true)
                    .qualityScore(70)
    ...
    private ValidateResult parseResponse(String response) {
        try {
            ...
            log.error("解析ValidateAgent响应失败: response={}, error={}", response, e.getMessage());
